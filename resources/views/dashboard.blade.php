...
<div class="app-shell">
    <aside class="sidebar">
        ...
        <nav aria-label="Main navigation">
            ...
            <button class="nav-item admin-only" data-tab="providers" hidden><span aria-hidden="true">⌘</span> Data sources <i>05</i></button>
        </nav>
    ...
    <div class="main-shell">
        <header class="topbar">
            ...
            <div class="topbar-actions"><button id="theme-toggle" class="button secondary theme-toggle" type="button" aria-label="Switch to light theme"><span aria-hidden="true">☀</span><span>Light</span></button><label class="sr-only" for="company">Company</label><select id="company" hidden></select><button id="account" class="account-button">Sign in <span class="avatar">→</span></button></div>
        </header>
...
<dialog id="auth-dialog" aria-label="Sign in"><form id="login-form"><div class="dialog-heading"><div><div class="eyebrow">YOUR WORKSPACE</div><h2>Welcome to Bolum.</h2></div><button type="button" class="icon-button" data-close aria-label="Close">×</button></div><p>Sign in to generate predictions and follow your results.</p><label>Email<input name="email" type="email" autocomplete="username" required></label><label>Password<input name="password" type="password" autocomplete="current-password" required></label><div class="form-error" role="alert"></div><button class="button primary full" type="submit">Sign in →</button>@if(app()->environment('local'))<p class="demo-hint">Local sample accounts:<br><strong>admin@bolum.test</strong> or <strong>member@bolum.test</strong><br>Password: <strong>password123</strong></p>@endif<p>Need a workspace? <button class="text-button" type="button" id="show-register">Create an account</button></p></form></dialog>
<dialog id="register-dialog" aria-label="Create your workspace"><form id="register-form"><div class="dialog-heading"><h2>Create your workspace</h2><button type="button" class="icon-button" data-close aria-label="Close">×</button></div><label>Your name<input name="name" required maxlength="100" autocomplete="name"></label><label>Company name<input name="company_name" required maxlength="100"></label><label>Email<input name="email" type="email" required autocomplete="username"></label><label>Password<input name="password" type="password" required minlength="10" autocomplete="new-password"></label><label>Confirm password<input name="password_confirmation" type="password" required minlength="10" autocomplete="new-password"></label><div class="form-error" role="alert"></div><button type="submit" class="button primary full">Create workspace →</button></form></dialog>
